...
public:
	...
	addr_t mul0_start_addr = 0x73000000;
 	addr_t mul0_size = 0x01000000;
 	addr_t mul0_end_addr = mul0_start_addr + mul0_size - 1;
	addr_t mul1_start_addr = 0x83000000;
 	addr_t mul1_size = 0x01000000;
 	addr_t mul1_end_addr = mul1_start_addr + mul1_size - 1;
	addr_t mul2_start_addr = 0x93000000;
 	addr_t mul2_size = 0x01000000;
 	addr_t mul2_end_addr = mul2_start_addr + mul2_size - 1;
	addr_t mul3_start_addr = 0xA3000000;
 	addr_t mul3_size = 0x01000000;
 	addr_t mul3_end_addr = mul3_start_addr + mul3_size - 1;
	...
	void parse(int argc, char **argv) override {
		...
		assert((mem_end_addr < clint_start_addr || mem_start_addr > mul0_end_addr) &&
...
int sc_main(int argc, char **argv) {
	...
	MemoryDMI dmi1 = MemoryDMI::create_start_size_mapping(mem.data, opt.mem_start_addr, mem.size);
	InstrMemoryProxy instr_mem0(dmi, core0);
	InstrMemoryProxy instr_mem1(dmi1, core1);
	...
	instr_memory_if *instr0_mem_if = &core0_mem_if;
	data_memory_if *data0_mem_if = &core0_mem_if;
	instr_memory_if *instr1_mem_if = &core1_mem_if;
	data_memory_if *data1_mem_if = &core1_mem_if;
	...
	if (opt.use_data_dmi) {
		...
		core1_mem_if.dmi_ranges.emplace_back(dmi1);
	}
	...
	core0.init(instr0_mem_if, data0_mem_if, &clint, entry_point,
	...
	bus.ports[12] = new PortMapping(opt.mul0_start_addr, opt.mul0_end_addr);
	bus.ports[13] = new PortMapping(opt.mul1_start_addr, opt.mul1_end_addr);
	bus.ports[14] = new PortMapping(opt.mul2_start_addr, opt.mul2_end_addr);
	bus.ports[15] = new PortMapping(opt.mul3_start_addr, opt.mul3_end_addr);
	//bus.ports[13] = new PortMapping(opt.mul1_start_addr, opt.mul1_end_addr);
	...
	bus.isocks[12].bind(mul0.tsock);
	bus.isocks[13].bind(mul1.tsock);
	bus.isocks[14].bind(mul2.tsock);
	bus.isocks[15].bind(mul3.tsock);
